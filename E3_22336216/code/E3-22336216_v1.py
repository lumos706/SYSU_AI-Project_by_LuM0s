...
class Puzzle:
    ...
    def successors(self):
        ...
        for i in range(len(move)):
            l = list(self.state)
            l[zero], l[zero + move[i]] = l[zero + move[i]], l[zero]  # swap the blank and the number
            puz_child = Puzzle(l)
...
def search(states, g, limit, visited):
    ...
    使用深度优先搜索解决拼图问题。
    参数:
        states: 一个列表，包含当前路径的所有状态的Puzzle实例。
        g: 一个整数，表示当前路径的成本。
    ...
    for successor in node.successors():
        if successor not in states and successor.state not in visited:
            ...
            states.pop()
            visited.remove(successor.state)  # remove the last element
    return min
